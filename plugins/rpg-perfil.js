import { xpRange } from '../lib/rpg-perfil.js'
...
const readSessionConfig = (conn) => {
  try {
    ...
    const botId = botJid?.split?.('@')?.[0]?.replace(/\D/g, '')
    if (!botId) return {}
...
let handler = async (m, { conn, args }) => {
  try {
    ...
    let userId = texto && texto.length > 0 ? texto[0] : (m.quoted ? await m.quoted.sender : m.sender)

    // read session config to get currency / bot name (works per-subbot)
    const cfg = readSessionConfig(conn)
    const currency = cfg.currency || global.currency || 'Coins'
    const nombreBot = cfg.name || global.botname || 'hola'

    // ensure db structures
    if (!global.db) global.db = { data: { users: {}, characters: {} } }
    if (!global.db.data) global.db.data = { users: {}, characters: {} }
    if (!global.db.data.users) global.db.data.users = {}
    if (!global.db.data.characters) global.db.data.characters = {}

    if (!global.db.data.users[userId]) global.db.data.users[userId] = {}

    let name = (global.db.data.users[userId].name) || (await (async () => {
      try {
